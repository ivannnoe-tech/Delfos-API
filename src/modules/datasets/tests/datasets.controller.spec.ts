...
describe('DatasetsController', () => {
  ...
  let baseUrl: string;
  let datasetsService: Pick<DatasetsService, 'create' | 'findByFilters'>;
  ...
  beforeAll(async () => {
    ...
    } as unknown as Pick<DatasetsService, 'create' | 'findByFilters'>;
  ...
  it('requires auth for list endpoint', async () => {
    const response = await fetch(`${baseUrl}/api/v1/datasets?tenantId=662d4f6e7a1c2b00124f0001`);
  ...
  it('creates dataset without returning sensitive metadata from response mock', async () => {
    const response = await fetch(`${baseUrl}/api/v1/datasets`, {
      method: 'POST',
      headers: {
        'content-type': 'application/json',
